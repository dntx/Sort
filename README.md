...
The explorer now includes an explicit `Enable Greedy Tighten` checkbox in the
...
Regression coverage lives in the `TopKFinder.Tests` project: `GreedyFeasibleStageTests.cs`,
`GreedyPipelineTests.cs`, `GreedyTightenTests.cs`, and `ProofTightenTests.cs`.
