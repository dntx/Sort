...
`greedy-feasible` / `proof-tighten≤N` / `edge compact exact` / `edge compact greedy`) and shows live
...
- `edge compact greedy`: one min-edge pass at the final feasible step `S`.
