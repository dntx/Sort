...
- **`TopKFinder.Tests`**：默认全程运行的单元 + 回归测试（当前约 **440+** 个已展开用例；
  ...
  护栏都在这里。
- **`TopKFinder.PerfTests`**：其中重型 / 按需诊断测试打 `Trait("Category", "Slow")`；在仓库默认过滤
  （`Category!=Slow`）下会被跳过，需显式参数运行。轻量墙钟烟雾测试（`StrategyPerformanceTests`）
  仍会默认运行。
...
> 注：本节这些重型诊断测试类都标记为 `Category=Slow`。若不带 `--filter` 直接执行 `dotnet test`，它们会被仓库默认过滤跳过。
