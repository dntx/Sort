...
- **`TopKFinder.Tests`**：默认全程运行的单元 + 回归测试（约 240+ 个）。正确性 oracle 与确定性计数器
  护栏都在这里。
...
- **`Default_IterativeDeepeningBaselineRemainsStable`**（14/16/17/18,5,5）：锁定 ID 路径自己物化的树形
  （MaxStep / 根组 / 边数 / 输出状态）+ 工作量计数器（searched / outcomes / candidate groups）。其中
  `OutcomesConstructed` 被明确指定为这条前沿的**主时间代理**。
