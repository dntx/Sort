...
- 模式与 CLI 一致：
  - `exact`：`step-proof → edge compact exact`
  - `greedy`：`greedy-feasible → (optional) greedy-tighten → proof-tighten≤N (0..n 次) → edge compact greedy`
...
- `proof-tighten≤N`
- `edge compact exact`（exact 终段）
- `edge compact greedy`（greedy 终段）
