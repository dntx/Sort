...
- **两种模式、各若干阶段**：编排层提供两条互斥的流水线，CLI 用 `--mode exact|greedy`、GUI 用下拉框切换。每个阶段都有一个
  ...
  - **greedy 模式（快速，feasibility-first）**：`greedy-feasible` = 构造式 feasible `BuildGreedyFeasiblePlan`（可行上界 `U`）。随后
    `BuildGreedyTightenPlan` 分两段：**Phase A** 用**只判可行、不计边数**的 compact 探测依次以 `U−1, U−2, …` 为根预算
    ...
    **compact 计划不会比 step 更差**。若 `BuildFeasibleCompactPlan` 被独立调用（builder 上没有先跑 step），则内部先跑一遍
    `BuildFeasiblePlan()` 自行确立 `U`。在大 `m`
  ...
  - **Anytime 呈现**：`BuildGreedyTightenPlan(onStage)` 接受一个回调，在**每次**产出一个阶段结果时**同步**触发——
    回调参数是 `GreedyTightenStage`（阶段名 + 该阶段**自身**耗时 + 可空的计划 + `Outcome` 枚举）。`Outcome` 区分三种结局：
