...
- **可能比 default 还差，故有兜底**：正因为上述代理偏差，在个别状态上 compact 反而渲染出**更多**边
  （例如 `10,4,8`：8 → 10）。所以 `BuildCompactPlan` 最后取 **compact 与 default 两者中边数更少的那个**
  （`compact.TotalBranchEdges < fallback.TotalBranchEdges ? compact : fallback`），保证**永不劣于 default**。
...
| **compact** | 精确最优（只在等优组里挑） | 经专门优化、**保证 ≤ default**，但因「代理最小化 + 兜底取 min」而**仍非可证明的全局最小** |

> 换言之：compact 之后的边数是「带安全兜底的启发式较优值」，把它称作「最优 edge-count」会过强。
