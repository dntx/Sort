...
display/search 两条物化路径当前通过独立 planner 入口（`BuildDisplayTransitionSpecs(...)` / `BuildSearchTransitionSpecs(...)`）生成过渡分支语义（顺序文本、effect、后继状态）；search 入口当前仍委托 display branch-line planner，以保证行为稳定并为后续继续解耦预留落点。
