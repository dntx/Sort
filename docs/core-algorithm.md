...
- `StrategyBuilder`（搜索层）负责求解语义、阶段可行性/最优性与不可变 `SolvedStrategy` 产物；兼容路径继续物化 `StrategyPlan`。
...
- exact projection and search-only exact entry both freeze a root-reachable `SolvedStrategy` and no longer depend on bridge-style nested entrypoints.
- shared exact session/bootstrap and shared expansion tracking are in place.
- display and search are independent consumers of frozen solver semantics; display is not the transport for exact search state.

display/search 两条物化路径当前通过独立 planner 入口（`BuildDisplayTransitionSpecs(...)` / `BuildSearchTransitionSpecs(...)`）生成过渡分支语义（顺序文本、effect、后继状态）；search 侧已切到 search-only branch payload（不再携带 display summary 字段），并进一步使用 search-only transition payload（不再复用 display 的 `TransitionSpec` 载体）。两侧共享 chosen-group line-planning 循环骨架，以及 branch-line->spec 的物化/排序骨架；branch-line 结果在 builder 内部也已统一为中性 line 模型，不再在共享路径上暴露 display 专用 `BranchLine` 类型。effect 字段改为先走共享原始字段计算，再分别生成 display/search effect 载体。代表分支选择已改为 display/search 各自规划：display 保留 summary 驱动渲染语义，search 仅保留代表选择所需逻辑；单轨道/多轨道判断从文本分隔符判定切换为语义判定，并去除中间选择载体结构，改为按语义分支直接构造输出。本轮进一步把 singleton line 的代表选择判定抽为 display/search 共用 helper，避免两侧出现语义分叉；同时把 display summary 构造延后到真正需要的路径，并在 singleton+projectionMerged 直接 relabel 的分支跳过 summary 构造，减少适配层无效计算。新增统一 relabel 分支构造 helper，并将 singleton-family 判定谓词与 projection-quotient 代表门控都集中为共享 helper，进一步压平 display/search 侧重复路径；随后移除已退化为冗余包装的 singleton 代表判定中间 helper，直接以语义谓词驱动分支选择。本轮进入下一类收口：将 display/search transition payload 的目标字段抽取统一为共享 raw helper，再分别生成 display/search effect 载体；并进一步将 target->spec 物化映射收敛到共享骨架，display 路径使用显式索引对齐保持 summary 绑定语义，并补齐 `BranchSpec`/`SearchBranchSpec` 的强类型目标字段抽取重载。继续收口后，display/search 两侧均通过同一个“带索引回调”的 target->spec 构造 helper 物化 transition payload。进一步进入后续类别时，`BuildBranchSpecs(...)` / `BuildSearchBranchSpecs(...)` 也改为共享“doomed-tail fallback + planned-line build + order sort”的规划骨架 helper，减少两条 planner 入口重复；并去除该共享 helper 上仅透传不参与判定的 `remainingSlots` 冗余参数。doomed-tail 视图也已改为先产出共享 line 模型，再分别投影到 display/search payload，以降低后续拆分时语义漂移风险。branch-line 划分策略仍委托 display planner，以保证行为稳定并为后续继续解耦预留落点。
...
- **可能比 default 还差，交由编排层裁决**：正因为上述代理偏差，在个别状态上 compact 反而渲染出**更多**边
  （例如 `10,4,8`：8 → 10）。`ExecuteEdgeCompactStage` **直接返回这个原始 compact 候选**（不再在 builder 内部跑第二遍
...
- **两种模式、各若干阶段**：编排层提供两条互斥的流水线，CLI 用 `--mode exact|greedy`、GUI 用下拉框切换。每个阶段都有一个
  ...
  - **exact 模式（默认）**：`step-proof` = 精确求解 `ExecuteStepProofStage`（已证明最优），`exact-edge-compact@S` = compact `ExecuteEdgeCompactStage`。
    **不跑可行 feasible**。step-proof 的首阶段已是步最优，故其 edge-compact 永远无法再降低步数，**没有**向下收紧阶段。
  - **greedy 模式（快速，feasibility-first）**：`greedy-feasible` = 构造式 feasible `ExecuteGreedyFeasibleStage`（可行上界 `U`）。随后可选
    `greedy-tighten` 预阶段：先做 root-only 微探针（`ShouldRunGreedyTightenByRootProbe`），仅当探针判断根层存在降高机会时才运行单轮
    `ExecuteGreedyTightenStage`；若其结果严格优于 `greedy-feasible`，则用 `OverrideGreedyPipelineUpperBound` 把 proof-tighten 的起始上界
    改为更紧的 `U'`。再进入
    ...
    min-edge** compact（`ProbeFeasibleCompact(S)`，发一个 `greedy-edge-compact@S` 阶段）以最小化边数。若这一步恰好被
    ...
    当前实现中 `greedy-feasible` 首阶段默认固定使用 base antichain 构造分组（不做 lookahead 评分）；
    `greedy-tighten` 里的 fallback 构造选择器仍保持默认行为。
    ...
    **compact 计划不会比 step 更差**。若 `RunGreedyPipeline` 被独立调用（builder 上没有先跑 step），则内部先跑一遍
    `ExecuteGreedyFeasibleStage()` 自行确立 `U`。在大 `m`
  ...
  - **向下预算收紧（默认开启、无时间上限、可中断）**：step 阶段给出的 `U` 系统性地偏大（实测几乎总是 `opt+1`）。
    ...
    **无时间上限**：收紧循环只在触达 `L`、证明 `NoSolution`、遇到 `Incomplete`、候选未变优、或**用户取消**时停止。
    ...
    同时支持 `list_only` 预检模式，可只验证 profile 选择器与 preflight 匹配结果而不执行测试/快照重跑。
    每次运行还会输出 matched-tests artifact（`counter-guardrails-matched-tests`），用于追踪 profile 选择器实际覆盖的测试清单。
    ...
    当前 compact-path 的 work-counter 侧也已补齐剩余 snapshot-only 行，包含 `(12,3,3)`、`(8,4,2)`、`(10,3,5)`、`(13,4,3)`，从而避免“主要 compact 成本代理只在 snapshot 表中可见”的尾部空档。
    在此基础上，nightly deterministic audit 现可直接复用同一 bundled full-audit 流程做无人值守巡检，让 matched-tests drift 与 snapshot 正向 delta 不必只靠手动触发才被发现。
  ...
  - **Anytime 呈现**：`RunGreedyPipeline(onStageCompleted)` 接受一个回调，在**每次**产出一个阶段结果时**同步**触发——
    ...
    **呈现以「是否
    ...
    `proof-tighten≤N` 与 `greedy-edge-compact@S` 之间的探测过渡，也用于首阶段（`greedy-feasible` / `step-proof`）尚未产出首棵树时的
...
> `ShouldRunGreedyTightenByRootProbe`，仅在 probe 通过时运行单轮 `ExecuteGreedyTightenStage`；且仅当 GT 结果严格优于
...
```text
greedy-feasible(U) → (optional) greedy-tighten → proof-tighten≤N → greedy-edge-compact@S
```
...
**实测 2 —— exact（`ExecuteStepProofStage` phase-1 纯搜索，Release，同 `maxStep`）**：
