...
当前 exact 入口已收口为单次 solver session：`BuildDisplayTreeAndExpandedSearch()` 在同一会话里产出 display/search 两个视图，`BuildSearchTree()` 则走独立的 search-only exact 入口并复用同一套 phase-1 缓存；两条路径都由 `StrategyBuilder` 基于 solver 状态递归直接构建 search tree。
